...
export function getAppConfig(): AppConfig {
  ...
  const appName = extra?.appName ?? process.env.EXPO_PUBLIC_APP_NAME ?? 'PakiSHIP Operator';
  const environment = resolveEnvironment(extra?.environment ?? process.env.EXPO_PUBLIC_APP_ENV);
  const apiBaseUrl = extra?.apiBaseUrl ?? process.env.EXPO_PUBLIC_API_BASE_URL ?? 'https://api.pakiship.ph';

  return { appName, environment, apiBaseUrl };
}
