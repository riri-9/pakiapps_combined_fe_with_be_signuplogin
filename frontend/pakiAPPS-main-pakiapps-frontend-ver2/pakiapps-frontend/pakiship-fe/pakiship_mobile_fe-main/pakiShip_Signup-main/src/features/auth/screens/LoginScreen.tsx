...
type DevLoginShortcut = {
  route: 'CustomerHome' | 'DriverHome' | 'OperatorHome';
  user: AuthUser;
};

const DEV_LOGIN_SHORTCUTS: Record<string, DevLoginShortcut> = {
  'customer@pakiship.com': {
    route: 'CustomerHome',
    user: {
      id: 'dev-customer',
      email: 'customer@pakiship.com',
  ...
  'driver@pakiship.com': {
    ...
    user: {
      id: 'dev-driver',
      email: 'driver@pakiship.com',
      fullName: 'Demo Driver',
      role: 'driver',
    },
  },
  'operator@pakiship.com': {
    route: 'OperatorHome',
    user: {
      id: 'dev-operator',
      email: 'operator@pakiship.com',
...
export default function LoginScreen({ onBackToLauncher }: LoginScreenProps) {
  ...
  const handleContinue = async () => {
    ...
    const devShortcut = DEV_LOGIN_SHORTCUTS[normalizedEmail];

    if (devShortcut) {
      setLoginError('');
      setCurrentUser(devShortcut.user);
      navigation.reset({
        index: 0,
        routes: [{ name: devShortcut.route }],
      });
      return;
    }
