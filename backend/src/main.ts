...
async function bootstrap(): Promise<void> {
  ...
  const mobileBffAlias: RequestHandler = (request, _response, next) => {
    if (
      request.url.startsWith('/pakiship/mobile') ||
      request.url.startsWith('/internal/')
    ) {
      request.url = `/api/v1${request.url}`;
    }

    next();
  };
  app.use(mobileBffAlias);
  ...
  if (enableSwagger) {
    const swaggerConfig = new DocumentBuilder()
      .setTitle('Garnet Mobile Backend')
      .setDescription('Garnet mobile BFF API')
      .setVersion('1.0.0')
      .build();
